...
class user
{
    ...
    function InsetUser($tenkh, $user, $matkhau, $email, $diachi, $dt)
    {
        ...
        if ($select) {
            $query = "insert into khachhang1( tenkh, username, matkhau, email, diachi, sodienthoai, vaitro) 
            values( '$tenkh', '$user', '$matkhau', '$email', '$diachi', '$dt', default)";
            $db->exec($query);
        } else {
        }
    }
    ...
    function insertcomment($mahh, $makh, $noidung)
    {
        ...
        $query = "insert into binhluan1( mahh, makh, ngaybl, noidung) 
        values( $mahh, $makh, '$datecreate', '$noidung')";
        echo $query;
        $db->exec($query);
    }
